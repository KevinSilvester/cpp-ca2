...
int createWindow(HINSTANCE &hInstance, int &nCmdShow, HMENU &hMenubar)
{
   ...
   HWND hWnd = CreateWindowEx(WS_EX_OVERLAPPEDWINDOW, szWindowClass, szTitle, WS_OVERLAPPEDWINDOW, CW_USEDEFAULT,
                              CW_USEDEFAULT, 300, 400, NULL, hMenubar, hInstance, NULL);
...
void dblBuffer(HWND hWnd, HDC hdc, LPRECT rcClientRect, Image *image)
{
   ...
   HBITMAP bmp =
      CreateCompatibleBitmap(hdc, rcClientRect->right - rcClientRect->left, rcClientRect->bottom - rcClientRect->top);
   ...
   BitBlt(hdc, 0, 0, rcClientRect->right - rcClientRect->left, rcClientRect->bottom - rcClientRect->top, memDC, 0, 0,
          SRCCOPY);
